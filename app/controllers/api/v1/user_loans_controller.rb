class Api::V1::UserLoansController < Api::BaseController
  ...
  def show
    ...
    render json: UserLoanSerializer.new(@user_loan, { include: ["user_account.includes(:user_loans).where(user_loans: { loan_id: #{@user_loan.loan_id} })", "user_account.transactions.where( loan_id: #{@user_loan.loan_id})"] } )
  end
